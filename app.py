...
def login():
    if request.form.get('password') == 'admin':
        ...
        return render_template('admin.html', logged_in=False, error="Senha incorreta!")
...
def admin():
    ...
    return render_template('admin.html', logged_in=True, players=players)
def on_connect():
...
if __name__ == '__main__':
    ...
    print(f"Para jogar no celular, use o IP da sua máquina (ex: http://192.168.15.20:5500)")
    
    # Pega a porta do ambiente (Render) ou usa 5500 se não tiver (Local)
    port = int(os.environ.get("PORT", 5500))
    # Debug só se não estiver no Render (opcional, mas boa prática)
